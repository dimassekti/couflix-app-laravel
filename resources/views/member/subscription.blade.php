...
@section('content')
  ...
  <div class="flex items-center gap-3">
    <img src="{{ asset('stream/assets/images/ic_subscription.svg') }}"
         alt="">
  ...
  <div class="flex flex-col gap-6 font-medium text-base text-white -mt-[10px] px-[18px]">
    <div class="flex gap-4 items-center">
      <img src="assets/images/ic_check-dark.svg"
           alt="">
      <span>{{ ucwords($user_premium->package->max_users) }} Users Limits</span>
    </div>
    <div class="flex gap-4 items-center">
      <img src="{{ asset('stream/assets/images/ic_check-dark.svg') }}"
           alt="">
  ...
  <div class="flex flex-col gap-[14px] max-w-max">
    <form method="POST"
          action="{{ route('member.transaction.store') }}">
      @csrf
      <input type="hidden"
             value="{{ $user_premium->package_id }}"
             name="package_id">
      <button type="submit"
  ...
  <div class="rounded-2xl bg-[#19152E] p-[30px] w-max">
    ...
    <form action="{{ route('member.user_premium.delete', $user_premium->id) }}"
          method="POST">
      @csrf
      @method('delete')
      <button class="px-[19px] py-[13px] bg-[#FE4848] rounded-full text-center">
        <span class="text-white font-semibold text-base">
          Stop Subscribe
        </span>
      </button>
